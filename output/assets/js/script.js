...
function printTable() {
  ...
  let priceTo = ([...tempData[0]]).sort((a, b) => getP(a) - getP(b)).map(p => p.split(' - ').reverse());
  let priceFrom = ([...tempData[1]]).sort((a, b) => getP(a) - getP(b)).map(p => p.split(' - ').reverse());
...
function openReturns(e, times, priceTo, priceFrom) {
  ...
  let r = priceTo.findIndex(t => t[0] == day);
  ...
  let days = times[1].filter((t, i) => (i >= d - (reverse ? options.show - 1 : 0)) && (i < d + (reverse ? 1 : options.show)));
